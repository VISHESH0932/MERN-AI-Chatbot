...
const getCookieOptions = (includeExpires: boolean = false) => {
  const options: any = {
    path: "/",
...
export const userSignup = async (
  ...
  try {
    ...
    if (existingUser) return res.status(409).send("User already registered"); // 409 Conflict is more appropriate
